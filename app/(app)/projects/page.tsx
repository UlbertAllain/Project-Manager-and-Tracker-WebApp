  ...
  return (
    <section className="space-y-5">
      <header className="page-heading">
        <div><span className="eyebrow"><FolderKanban className="size-3.5" /> PORTOFOLIO PROYEK</span><h2>{query ? `Hasil pencarian “${q.trim()}”` : "Proyek"}</h2><p>{query ? `${projects.length} proyek ditemukan dari ${accessibleProjects.length} proyek yang dapat kamu akses.` : "Proyek internal yang dapat kamu akses beserta kondisi pekerjaan tim."}</p></div>
      ...
      <div className="project-grid">
          ...
          return (
            <article className="project-card" key={project.id}>
              <div className="project-card-top">
                <div className="flex flex-wrap items-center gap-2"><span className={`health-badge ${health.tone}`}>{health.label}</span><span className="badge">{PROJECT_STATUS_LABELS[project.status]}</span>{canManageProject(user, project) ? <span className="manager-chip">Kamu mengelola</span> : null}</div>
              ...
              <Link href={`/projects/${project.id}`} className="block">
                ...
                <p className="project-objective">{project.objective || project.description || "Tujuan project belum dituliskan."}</p>
                <div className="project-progress"><div><span>Progress</span><strong>{project.progress}%</strong></div><div className="progress-track"><span style={{ width: `${project.progress}%` }} /></div></div>
                <div className="project-card-stats"><div><strong>{summary.total}</strong><span>Total task</span></div><div><strong>{summary.review + summary.revision}</strong><span>Review/revisi</span></div><div><strong>{summary.blocked}</strong><span>Blocked</span></div></div>
        ...
        {projects.length === 0 ? <div className="panel-card empty-state col-span-full">Belum ada proyek yang dapat diakses.</div> : null}
      </div>
    </section>
  );
}
