...
export default async function FinancePage() {
  ...
  return (
    <section className="space-y-5">
      <header className="page-heading"><div><span className="eyebrow"><Landmark className="size-3.5" /> KONTROL PEMILIK</span><h2>Keuangan</h2><p>Ringkasan pemasukan, pengeluaran, dan margin proyek yang hanya dapat diakses admin.</p></div></header>
      ...
      <section className="panel-card overflow-hidden">
        <div className="section-header"><div><span className="eyebrow">RIWAYAT TRANSAKSI</span><h3>Riwayat transaksi</h3><p>Setiap perubahan mengoreksi total proyek secara atomik.</p></div><span className="count-pill">{transactions.length}</span></div>
        <div className="table-wrap"><table><thead><tr><th>Tanggal</th><th>Proyek</th><th>Jenis</th><th>Keterangan</th><th>Nominal</th><th /></tr></thead><tbody>{transactions.map((transaction) => <tr key={transaction.id}><td>{formatDate(transaction.date)}</td><td className="font-semibold text-[var(--text-main)]">{transaction.projectName}</td><td><span className={`health-badge ${transaction.type === "INCOME" ? "success" : "danger"}`}>{transaction.type === "INCOME" ? "Pemasukan" : "Pengeluaran"}</span></td><td>{transaction.description}</td><td>{formatCurrency(transaction.amount)}</td><td><form action={deleteTransactionAction}><input type="hidden" name="transactionId" value={transaction.id} /><ConfirmSubmitButton className="task-delete" message="Hapus transaksi ini dan koreksi saldo proyek?">Hapus</ConfirmSubmitButton></form></td></tr>)}{transactions.length === 0 ? <tr><td colSpan={6}><div className="empty-state">Belum ada transaksi.</div></td></tr> : null}</tbody></table></div>
      </section>
    </section>
  );
}
...
function Metric({ icon, label, value, danger = false }: { icon: React.ReactNode; label: string; value: string; danger?: boolean }) {
  return <div className={`dashboard-metric ${danger ? "danger" : ""}`}><div className="dashboard-metric-icon">{icon}</div><div><p>{label}</p><strong>{value}</strong><span>Seluruh project</span></div></div>;
}
