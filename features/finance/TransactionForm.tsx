...
export function TransactionForm({ projects }: { projects: Project[] }) {
  return (
    <section className="panel-card overflow-hidden">
      <div className="section-header"><div><span className="eyebrow">FINANCE ENTRY</span><h3>Catat transaksi project</h3><p>Hanya admin/owner yang dapat melihat dan mengubah data keuangan.</p></div></div>
      <form action={addTransactionAction} className="finance-form">
        <div><label className="label">Project</label><select className="input" name="projectId" required><option value="">Pilih project</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></div>
        ...
        <div><label className="label">Keterangan</label><input className="input" name="description" placeholder="DP, hosting, domain..." required /></div>
